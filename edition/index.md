...
<header id="main-header">
  ...
  <div id="search-bar" class="hidden">
    <input
      type="text"
      id="search-input"
      placeholder="Buscar por término, frase o AND/OR/NOT"
    />
...
<script>
  ...
  e.stopPropagation();
  document
  .querySelectorAll(".floating-note:not(.pinned)")
  .forEach((n) => n.remove());
  const rect = e.target.getBoundingClientRect();
  const floating = document.createElement("div");
  floating.className = "floating-note";
  floating.innerHTML = `
          <div class="floating-note__controls">
            <button class="pin-note" type="button" title="Pin note" aria-label="Pin note" aria-pressed="false">📌</button>
            <button class="close-note" type="button" title="Close note" aria-label="Close note">✕</button>
          </div>
          <div class="floating-note__body">${content}</div>`;
  document.body.appendChild(floating);
  const maxLeft = Math.max(12, window.innerWidth - 320);
  const maxTop = Math.max(12, window.innerHeight - 220);
  floating.style.left = `${Math.min(Math.max(12, rect.right + 10), maxLeft)}px`;
  floating.style.top = `${Math.min(Math.max(12, rect.top), maxTop)}px`;
  const pinBtn = floating.querySelector(".pin-note");
  const closeBtn = floating.querySelector(".close-note");
  pinBtn.addEventListener("click", (ev) => {
  ev.stopPropagation();
  const isPinned = floating.classList.toggle("pinned");
  ...
  floating.addEventListener("pointerdown", (ev) => {
  if (ev.target.closest(".floating-note__controls")) {
  return;
  }
  ev.preventDefault();
  ...
  floating.style.left = `${Math.min(Math.max(12, startLeft + moveEvent.clientX - startX), Math.max(12, window.innerWidth - 320))}px`;
  floating.style.top = `${Math.min(Math.max(12, startTop + moveEvent.clientY - startY), Math.max(12, window.innerHeight - 220))}px`;
  };
  const handleUp = () => {
  ...
  if (e.target.closest(".floating-note")) {
  return;
  }
  document.querySelectorAll(".floating-note:not(.pinned)").forEach((n) => n.remove());
  });
  }
  ...
  let modernLoaded = false;
      ...
      function buildQueryPlan(query) {
        ...
        tokens.forEach((token) => {
          ...
          const phrase = token.startsWith('"') && token.endsWith('"');
          const value = phrase ? token.slice(1, -1) : token;
          if (!value) return;

          terms.push({
            value,
            phrase,
            operator: pendingOperator || "TERM",
          });
        ...
        return {
          type: /\b(AND|OR|NOT)\b/i.test(trimmed) ? "boolean" : "simple",
          terms,
        };
      }

      function matchesQueryPlan(plan, text) {
        if (!plan || !text) return false;

        const normalized = text.toLowerCase();
        if (plan.type === "simple") {
          const term = plan.terms[0];
          return normalized.includes(term.value.toLowerCase());
        }

        let result = false;
        let pendingOperator = "AND";
        plan.terms.forEach((term, index) => {
          const termMatches = normalized.includes(term.value.toLowerCase());
          const effectiveMatch = term.operator === "NOT" ? !termMatches : termMatches;
          ...
          if (term.operator !== "TERM") {
            pendingOperator = term.operator;
          }
        });
      ...
      function buildHighlightRegex(plan) {
        if (!plan) return null;
        const values = plan.terms
          .map((term) => term.value)
          .filter(Boolean)
      ...
      function highlightBlockMatches(block, plan, scope) {
        if (!block || !plan) return [];
        const regex = buildHighlightRegex(plan);
        if (!regex) return [];
        ...
        textNodes.forEach((textNode) => {
          ...
          for (let i = 0; i < split.length; i++) {
            ...
            if (i < split.length - 1) {
              ...
              span.textContent = split[i + 1];
              fragments.push(span);
              matches.push({ el: span, scope, blockIndex: block.dataset.searchBlockIndex });
            }
          }
      ...
      function highlightText(container, query, results = [], scope) {
        ...
        const blocks = Array.from(
          container.querySelectorAll(HIGHLIGHT_BLOCK_SEL),
        );
        blocks.forEach((block, index) => {
          block.dataset.searchBlockIndex = String(index);
          if (!matchesQueryPlan(plan, block.textContent || "")) return;
          const matches = highlightBlockMatches(block, plan, scope);
      ...
      function findPairedResult(result) {
        return searchResults.find(
          (candidate) =>
            candidate !== result &&
            candidate.scope !== result.scope &&
            candidate.blockIndex === result.blockIndex,
        );
      }
      ...
      function scrollToMatch(index) {
        ...
        const currentMatch = searchResults[currentResultIndex];
        currentMatch.el.classList.add("active-match");

        const pairedMatch = findPairedResult(currentMatch);
        if (pairedMatch) {
          pairedMatch.el.classList.add("active-match");
        }

        const containers = [
          currentMatch.el.closest(".tei-column-scroll"),
          pairedMatch?.el.closest(".tei-column-scroll"),
        ].filter(Boolean);
        ...
        containers.forEach((container) => {
          ...
          const mRect = currentMatch.el.getBoundingClientRect();
          const targetMatch =
            container === currentMatch.el.closest(".tei-column-scroll")
              ? currentMatch.el
              : pairedMatch?.el;
          if (!targetMatch) return;
          const tRect = targetMatch.getBoundingClientRect();
          const offsetWithin = tRect.top - cRect.top + container.scrollTop;
          container.scrollTo({ top: Math.max(offsetWithin - 40, 0), behavior });
        });

        if (typeof currentMatch.el.focus === "function") {
          currentMatch.el.focus({ preventScroll: true });
        }
